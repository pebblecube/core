...
if($index >= 0)
{
	if(is_array($prj_obj->achievements))
		if($index < sizeof($prj_obj->achievements))
			$board = $prj_obj->achievements[$index];
}
...
$results = achievements_manager::get_achievement_board($prj_obj->id, $board->code, $from, $to, $page_index, $page_size);
data::close_conn();
...
<body>
	...
	<div class="container container_12 standardcontent">
		...
		<div class="grid_12">
			<h2><?php echo(htmlspecialchars($prj_obj->achievements[$index]->title)); ?></h2>
			<ul>
				<?php 
